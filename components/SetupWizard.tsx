...
import { UserPlus, Shield, CheckCircle, ArrowRight, Building2, KeyRound } from 'lucide-react';
    ...
    <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100 max-w-md w-full animate-fade-in">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="space-y-4">
            <div>
                ...
                <input required type="text" className="w-full px-4 py-2 border rounded-lg" value={data.name} onChange={e => setData({...data, name: e.target.value})} />
            </div>
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Email Address</label>
                <input required type="email" className="w-full px-4 py-2 border rounded-lg" value={data.email} onChange={e => setData({...data, email: e.target.value})} />
            </div>
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Set Password</label>
                <input required type="password" className="w-full px-4 py-2 border rounded-lg" value={data.password} onChange={e => setData({...data, password: e.target.value})} />
            </div>
        ...
        <div className="mt-6 flex justify-center gap-2">
            {[1, 2, 3].map(i => (
                <div key={i} className={`h-1.5 w-8 rounded-full ${i === step ? 'bg-indigo-600' : 'bg-slate-200'}`} />
            ))}
        </div>
    </div>
  );
